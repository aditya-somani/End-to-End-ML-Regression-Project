...
Link to In-depth Notes (Intuition + answering "why" of everything) : [Link to Notes(Give it a try!)](https://www.notion.so/Build-Deploy-ML-Regression-model-with-FastAPI-MLFlow-Docker-AWS-29ec71575c4d80808961ec65256868a9?source=copy_link)
...
- **AWS S3 Integration**: Data and model storage in `housing-regression-data-aditya-somani-ml-project` bucket
...
#### ECS Services:
- **ml-api-service**: FastAPI backend (port 8000, 1024 CPU, 3072 MB memory)
- **streamlit-app-service**: Streamlit dashboard (port 8501, 512 CPU, 1024 MB memory)
...
- **`data/`**: Raw, processed, and prediction data (time-structured, S3-synced) - Not pushed due to size :)
