...
def add_date_features(df: pd.DataFrame) -> pd.DataFrame:
    """Add date parts from 'date' column."""
    if 'date' not in df.columns:
        print("Skipping date feature addition: no 'date' column present.")
        return df
...
def target_encode(
    ...
    encoded_col = f'{col}_encoded' if col!='city_full' else 'city_full_encoded' # {col}_encoded also works :)
    train[encoded_col] = encoder.fit_transform(train[col], train[target])
    eval[encoded_col] = encoder.transform(eval[col])
    return train, eval, encoder

def drop_unused_columns(
    train: pd.DataFrame,
    eval: pd.DataFrame,
) -> tuple[pd.DataFrame, pd.DataFrame]:
    """Drop columns that may cause leakage or are unused."""
    ...
    eval = eval.drop(columns=[c for c in drop_columns if c in eval.columns], errors='ignore')
    return train, eval
...
def run_feature_engineering(
...
) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.Series, TargetEncoder]:
    """
    ...
    if 'zipcode' in train_df.columns:
        train_df, eval_df, freq_map = frequency_encode(train_df, eval_df, col='zipcode')
        holdout_df['zipcode_freq'] = holdout_df['zipcode'].map(freq_map).fillna(0)
        dump(freq_map, MODELS_DIR / 'freq_encode.pkl') # save mapping
